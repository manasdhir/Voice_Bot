...
async def get_kb_documents(kb_name: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        logger.info(f"Retrieved {len(documents)} documents for knowledge base '{kb_name}' for user {user_id}")
        ...
        logger.error(f"Error fetching documents for knowledge base '{kb_name}': {str(e)}")
        raise HTTPException(
            status_code=500,
...
async def delete_document(kb_name: str, filename: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        if success:
            logger.info(f"Deleted document '{filename}' from knowledge base '{kb_name}' for user {user_id}")
        ...
        logger.error(f"Error deleting document '{filename}' from knowledge base '{kb_name}': {str(e)}")
        raise HTTPException(
            status_code=500,
