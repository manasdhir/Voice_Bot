...
async def create_persona(
    persona_data: CreatePersonaRequest, user=Depends(get_current_user)
):
    try:
        ...
        print(
            f"✅ Persona created successfully for user {user_id}: {created_persona['name']}"
        )
...
async def get_all_personas(user=Depends(get_current_user)):
    try:
        ...
        print(
            f"✅ Retrieved {result['total']} personas for user {user_id} ({result['default_count']} default, {result['user_count']} custom)"
        )
...
async def activate_persona(
    persona_id: str, request: SetActivePersonaRequest, user=Depends(get_current_user)
):
    ...
    try:
        ...
        print(
            f"✅ Activated {request.persona_source} persona {persona_id} for user {user_id}"
        )
...
async def deactivate_persona(user=Depends(get_current_user)):
...
async def update_persona(
    persona_id: str, request: UpdatePersonaRequest, user=Depends(get_current_user)
):
    try:
        user_id = user["sub"]
